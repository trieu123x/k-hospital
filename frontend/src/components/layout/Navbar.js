    ...
    <nav className="fixed h-15 w-full bg-white z-30">
      <div className="flex h-full xl:justify-between bg-[#070575] xl:bg-transparent">
        ...
        >
          ...
          {/* Center Links */}
          <div
            className={`hidden xl:flex items-center space-x-5 text-[20px] ${isAdmin ? "mr-[12%]" : "mr-[18%]"}`}
          >
            ...
            >
              <span>Tra cứu</span>
              <ChevronDown
                className={`size-3 ${isLookUpOpen && "rotate-180"}`}
              />
              {isLookUpOpen && (
                <div
                  ref={lookUpRef}
                ...
                >
                  <LinkButton
                    href="/diseases"
                    className="hover:bg-[#3040A8] rounded-none px-12 py-2"
                  >
                    Bệnh
                  </LinkButton>
                  <LinkButton
            ...
            <LinkButton href="/booking">Đặt lịch khám</LinkButton>

            {isAdmin && (
              <LinkButton href="/admin/aggregate">Quản lý</LinkButton>
            )}
          </div>
  ...
  return (
    <div className="hidden xl:flex w-1/5 justify-end items-center gap-2">
      <div className="relative cursor-pointer hover:bg-[#E8E8E8] transition-all duration-300 p-1 rounded-full">
        {hasUnread && (
          <div className="absolute -top-1 -right-1 size-2.5 bg-red-500 rounded-full border-2 border-white z-10"></div>
        )}
