...
export default function Detail() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        setAvatarFile(null);
        setNewCropData(null);
        // Có thể gán lại serverCropData = newCropData ở đây nếu muốn UI đồng bộ ngay, 
        // nhưng người dùng thường sẽ load lại trang.
      } else {
  ...
  if (isDoctor || isAdmin) return <div className="grow flex flex-col items-center justify-center bg-gray-50 min-h-[500px]">Truy cập bị từ chối!</div>;
    ...
    <div className="grow flex flex-col rasa-font bg-gray-50">
      <div className="relative grow flex px-10 py-8 justify-between gap-35">
        <div className="w-200 flex flex-col flex-none">
          ...
          {hasChanges() && (
            <Button onClick={handleSave} disabled={saving} className={`w-fit px-8 mt-3 bg-[#070575] text-white ${saving ? "opacity-70" : ""}`}>
