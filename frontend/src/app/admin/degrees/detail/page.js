...
import { getDegreeDetail, createDegree, updateDegree } from "@/routers/degree-api"
...
function DetailContent() {
  ...
  const [rankWeight, setRankWeight] = useState("")
      ...
      try {
        if (isEditMode) {
          ...
          if (res.data) {
            ...
            setRankWeight(dataToUse.rankWeight?.toString() || "")
            setDescription(dataToUse.description || "")

            setInitialData({
              name: dataToUse.name || "",
              rankWeight: dataToUse.rankWeight?.toString() || "",
        ...
        console.error("Lỗi lấy chi tiết bằng cấp:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        name,
        rankWeight: parseInt(rankWeight) || 0,
        description
      }

      if (isEditMode) {
        await updateDegree(id, payload)
      ...
      console.error("Lỗi lưu bằng cấp:", error)
      alert("Lưu thất bại!")
    } finally {
      hideLoading()
    }
    ...
    <div className="grow flex flex-col rasa-font bg-white">
      <div className="relative grow flex px-10 py-8 justify-between gap-35">
        <div className="w-200 flex flex-col flex-none">
          <InputForm label={"Tên bằng cấp"} placeholder={"Nhập tên bằng cấp"}
            value={name} setValue={setName} />
          <InputForm label={"Trọng số thứ bậc (Số)"} placeholder={"Nhập trọng số (VD: 90, 100)"}
            value={rankWeight} setValue={setRankWeight} />
