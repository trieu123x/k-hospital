...
import { getCategoryDetail, createCategory, updateCategory } from "@/routers/categorize-api"
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        console.error("Lỗi lấy chi tiết loại bệnh:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        router.push("/admin/disease-categories")
      }
    } catch (error) {
      console.error("Lỗi lưu loại bệnh:", error)
      alert("Lưu thất bại!")
    } finally {
      hideLoading()
    }
    ...
    <div className="grow flex flex-col rasa-font bg-white">
      <div className="relative grow flex px-10 py-8 justify-between gap-35">
        <div className="w-200 flex flex-col flex-none">
          <InputForm label={"Tên loại bệnh"} placeholder={"Nhập tên loại bệnh"}
