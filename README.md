...
<p align="center">
  ...
  Tích hợp AI Chatbot (RAG) và hệ thống phân tích dữ liệu ETL.
</p>

## 1. Giới thiệu

**MediCare (K-Hospital)** là một giải pháp chuyển đổi số toàn diện dành cho các cơ sở y tế và phòng khám. Dự án nhằm giải quyết bài toán ùn tắc tại bệnh viện, khó khăn trong tra cứu thông tin y tế và thiếu hụt dữ liệu phân tích hành vi bệnh nhân.
...
## 3. Kiến trúc hệ thống & Luồng dữ liệu (The Brains)

### Kiến trúc tổng thể

```mermaid
graph TD
    Client[Next.js Client] -->|REST API| Backend[Express.js Backend]
    Client -->|SSE| AI[FastAPI AI Service]
    
    Backend -->|Prisma ORM| DB[(PostgreSQL)]
    AI -->|pgvector| DB
    
    Backend -->|Auth & OTP Email| Supabase[Supabase Auth]
    
    DB -.->|Extract| ETL[ETL Pipeline - DuckDB]
    ETL -.->|Transform & Load| DataLake[(Parquet Files)]
...
### Luồng xử lý AI Chatbot (RAG Architecture)

AI Chatbot sử dụng kiến trúc **Retrieval-Augmented Generation (RAG)** kết hợp với `pgvector` để tìm kiếm thông tin y khoa chính xác trước khi đưa cho LLM (Gemini) sinh câu trả lời.

```mermaid
sequenceDiagram
    participant User
    participant AI_Service as FastAPI
    participant DB as PostgreSQL
    participant LLM as Google Gemini

    User->>AI_Service: Gửi câu hỏi triệu chứng
    AI_Service->>LLM: Viết lại và Phân loại Intent
    LLM-->>AI_Service: Trả về Intent và Query
    AI_Service->>AI_Service: Chuyển Query thành Vector
    AI_Service->>DB: Truy vấn KNN Cosine
    DB-->>AI_Service: Trả về các bệnh và bác sĩ phù hợp
    AI_Service->>LLM: Gửi Prompt kèm Context
    LLM-->>AI_Service: Sinh Text Streaming SSE
    AI_Service-->>User: Trả về câu trả lời
```

### Luồng dữ liệu ETL (Data Pipeline)

```mermaid
graph LR
    A[(PostgreSQL)] -->|Extract| B(DuckDB)
    B -->|Transform| C{Aggregations}
    C -->|Top Doctors| D[Report Storage]
    C -->|Peak Shifts| D
    C -->|Daily Summary| D
    D -->|Load| Admin[Admin Dashboard]
```

---

## 4. Phần công nghệ sử dụng (The Tech Stack)
...
## 5. Hướng dẫn cài đặt & Triển khai (The Execution)
...
## 6. Cấu hình Environment (Environment Variables)
